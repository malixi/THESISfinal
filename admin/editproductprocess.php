<?php
	...
	if($prodImage == NULL){
		...
		if($rows = $result->fetch_assoc()) {
			$stmt2 = $dbconn->prepare('UPDATE products SET name = ?, price = ?, description = ?, quantity = ?, date_created = ? WHERE productID = ?');
			$stmt2->bind_param('sdsisi', $prodName, $prodPrice, $prodDesc, $prodQty, $prodCreated, $prodID);
			$stmt2->execute();
